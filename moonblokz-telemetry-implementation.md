...
- **Consistency:** This document now treats `moonblokz-probe`, `moonblokz-telemetry-hub`, `moonblokz-log-collector`, `moonblokz-telemetry-cli`, and `moonblokz-telemetry-update-server` as implementation-grounded sources instead of leaving the update host and operator command surface at spec/doc-summary level.
- **Logical soundness:** The file preserves the current boundaries clearly: one USB owner and bounded delivery buffer on the Probe side, one SQLite/KV-backed request handler on the HUB side, one timestamp-cursor append-only extraction loop on the Collector side, one parser-plus-HTTP submission tool on the CLI side, and one static artifact-publication surface on the Update Server side.
- **Feasibility:** The described engineering model matches the repositories as they exist today, including their real operational dependencies, documented drift, and update-path/cloud-contract/collection/command-surface/artifact-hosting sharp edges.
