...
When several candidate segments satisfy a stopping condition at once, the node selects the one whose tip has the highest sequence, breaking a same-sequence tie by the lowest tip block hash (big-endian) — an in-memory, deterministic total order. This collecting-phase selection is only a bootstrapping choice of which candidate to reconstruct first: it does not determine the final authoritative chain. Once reconstruction reaches ready state, branch value (Algorithm 8 / branch selection below) governs the active branch, and a branch-value re-evaluation chain-switches away from the provisional pick if it was not the authoritative branch. Branch value cannot be used during acquisition because it is defined relative to an active-chain tail that does not yet exist. Aligning the collecting tie-break with the fuller ready-state rule (tip serialized size → creator node id → tip hash) is a deferred optimization that would avoid one otherwise-unnecessary chain-switch in the rare equal-sequence / equal-branch-value case; the MVP uses hash only, because block-tree metadata does not retain tip size or creator and the fuller rule would require reading tip block content. See `moonblokz-blockchain-prd.md` FR2.
...
Common to every structure below: each node identifier a transaction references — the initializer and receiver of a node transfer, the initializer of a registration, and each balance input's initializer and balance output's receiver of a complex transaction, alongside the `vote` field — must name a node that exists at the transaction's inclusion point. Because node identifiers are assigned with stride 1 and never reused, existence is decided by the `max_known_node_id` watermark alone (`node_id` within `[0, max_known_node_id]`), so the test holds even for a node whose balance state has not been re-derived from a within-window seed source. See [Blockchain PRD](./moonblokz-blockchain-prd.md) FR6 for the normative rule and its block-`#0` waiver.
