...
- **Consistency:** This document now reflects the current `moonblokz-radio-simulator` codebase rather than only the earlier article framing, while remaining aligned with the existing radio knowledge-base documents.
- **Logical soundness:** The file clearly separates current scope, deliberate simplifications, and codebase-driven functional expansion.
- **Feasibility:** The documented concept remains realistic for the implemented desktop application: one reusable radio core, bounded async execution, mode-based workflows, scene-driven context, and operator-facing visualization.
