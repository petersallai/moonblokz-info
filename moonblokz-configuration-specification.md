...
**The one bound no mechanism above can enforce** is the *relational* fee range, `tx_fee_per_byte_min ≤ tx_fee_per_byte_max`: no per-parameter predicate can see two values. Acceptance checks it on declared literals, substituting a code-baked default for an absent or program-valued parameter; when either fee is computed the relation is therefore unchecked. Its failure mode is a rule the whole chain applies alike, on node-#0-signed content.
