...
1. `max_block_utxo_output ≤ UTXO_UNSPENT_BITS` — the compile-time per-block UTXO spent-bit width (FR8). A larger value cannot be represented by the local node's cache. `UTXO_UNSPENT_BITS` is a `pub const` of the configuration crate. **Story 5.8 pins it to the blockchain's real spent-bit width with a monomorphization-time assertion; until then nothing does** — the blockchain does not depend on this crate before that story wires it in, so the pin cannot exist yet. The parameter is **two bytes wide for this bound's sake** (ratified 2026-08-19): while it was one byte a declared literal could not exceed 255, so with a 256-bit width the check could never fail and the top of the range could never be used. Two bytes make the out-of-range case representable, which is what a kind-2 bound needs — the refusal path exists for the node whose cache is smaller than the chain demands, and refusing is the whole remedy.
