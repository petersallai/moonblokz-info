...
1. `max_block_utxo_output ≤ UTXO_UNSPENT_BITS` — the compile-time per-block UTXO spent-bit width (FR8). A larger value cannot be represented by the local node's cache. `UTXO_UNSPENT_BITS` is a `pub const` of the configuration crate. **Story 5.8 pins it to the blockchain's real spent-bit width with a monomorphization-time assertion; until then nothing does** — the blockchain does not depend on this crate before that story wires it in, so the pin cannot exist yet. **The check is also dormant in the default build, and that is worth stating plainly:** the parameter is one byte wide, so a declared literal cannot exceed 255, while the default width is 256 — no legal content can fail it. It becomes live the moment a build narrows its spent-bit width below 255, which is exactly what makes this a kind-2 bound rather than dead code: the refusal path exists for the node whose cache is smaller than the chain demands, and Story 5.8 makes the width follow the const generic so that such a build is expressible at all.
2. `required_support ≥ 1` (FR8) — otherwise ADR-015's `m = min(2·required_support − 1, |A|)` yields `m = -1`.
3. *(withdrawn as an acceptance check, 2026-08-18)* `required_support ≤ MAX_AGGREGATED_SIGNATURES` is no longer checked here. The limit is per-build, so enforcing it on a computed value would diverge (kind 2 below); instead the chain declares the ceiling in `max_aggregated_signatures` — check 8, still measured against the backend — and the accessor clamps to it ([ADR-015](./blockchain-adrs/ADR-015-approval-subgroup-selection.md), §6 kind 3).
...
**1. A universal limit — enforced at acceptance and at resolution.** `vote_scale ≠ 0`, `required_support ≥ 1`, `block_fill_threshold_percent ≤ 100`, and the block format's own `HEADER_SIZE` / `MAX_BLOCK_SIZE`: the limit is a constant of the protocol, identical on every build. Acceptance checks a declared literal; §5.1's resolution guard checks a *computed* value with the same predicate, and a violation fails that tier. Such a parameter may therefore admit a program (IDs 6, 10, 20) without any node ever reading an illegal value, because every node reaches the same verdict on the same content.

**2. A limit that is a compile-time constant of the local build — acceptance only, so the parameter is literal-only.** `max_block_utxo_output ≤ UTXO_UNSPENT_BITS` (ID 4), `max_aggregated_signatures ≤ MAX_AGGREGATED_SIGNATURES` (ID 5), `active_chain_length ≤ SNAKE_CHAIN_LENGTH_MAX` (ID 21). At acceptance such a bound is safe *because* the response is refusal: a node that cannot honour the declared value rejects the chain and stops participating, and a node that is absent cannot diverge. As a resolution-time fallback the same bound would be unsafe — a fallback keeps the node participating with a *different value* than a differently-built node resolves, with no error on either side, which is the silent split of §3.4 reached through the bounds instead of the keys. These parameters are literal-only so that the decision cannot reach the resolution path at all; the crate asserts the form at compile time.
...
**The one bound no mechanism above can enforce** is the *relational* fee range, `tx_fee_per_byte_min ≤ tx_fee_per_byte_max`: no per-parameter predicate can see two values, so neither the acceptance check nor the resolution guard can be made total for it. Acceptance checks it on declared literals — substituting a code-baked default for an absent or program-valued parameter — which makes it a **founder-facing diagnostic**: `config-encoder` runs the same pass, so a typo in two declared literals is caught before a chain exists. It is not a guarantee, because a computed fee is unknowable at that point.

Ratified 2026-08-18: **the consumer resolves an inconsistent range at the point of use.** The fee policy that reads these two parameters must not assume `min ≤ max`, and whatever it does about an inverted range must be deterministic — every node reads the same content and must arrive at the same fee. Deliberately not solved with a clamp here: a clamp would pick one of the two values as authoritative, which is a fee-policy decision rather than a configuration one.
...
**A program cannot observe anything node-local or non-deterministic.** No wall-clock time, no randomness, no node-specific quantity — the local node's neighbour count, link qualities, queue depths, or storage occupancy — and no mempool content, which is network-held runtime state that never converges. Every input is an argument supplied by the caller, another configuration parameter, or (per the ratified §4.6 extension) a quantity derived from the active chain itself. The third class is admissible for the same reason the first two are: it is agreed content, so a value computed from it is a value the network can agree on. This is the same rule that makes radio parameters argument-less (§10.2), and for the same reason: a value that differs between nodes cannot be a value the network agrees on, and configuration exists precisely to be agreed on.
