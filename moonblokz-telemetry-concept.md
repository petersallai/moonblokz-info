...
- **Consistency:** This document now treats the Probe, HUB, Collector, CLI, and Update Server as code-grounded sources of truth for the reviewed station-side, cloud-side, local-extraction, operator-command, and artifact-hosting telemetry behavior instead of relying only on the article summary.
- **Logical soundness:** The file keeps the central architectural distinction clear: LoRa carries MoonBlokz network behavior, while USB + WiFi/cloud carry operational telemetry and control, the Collector remains a downstream local consumer, the CLI remains a hub-mediated control surface rather than a direct device channel, and the Update Server remains a static artifact host rather than a custom update-control engine.
- **Feasibility:** The documented model matches the currently implemented behavior: bounded station-side buffering, local persistence, periodic sync, explicit cloud persistence, global interval policy, delayed download serving, a simple timestamp-cursor-based Collector loop, a thin local CLI that parses then submits hub commands, and a static update host that publishes version metadata plus one current Probe and Node artifact.
