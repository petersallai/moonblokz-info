...
- **Consistency:** This document now reflects the actual `moonblokz-radio-simulator` crate structure, current modes, and real implementation techniques rather than only the article-era simulator framing.
- **Logical soundness:** The notes preserve the distinction between reusable radio-core behavior, environment-side simulation logic, analyzer reconstruction logic, and UI/control responsibilities.
- **Feasibility:** The described engineering model is realistic for the current desktop tool: bounded queues, bounded histories, host-side leaked allocations where needed, immediate-mode rendering, scene-driven context, and a practical control channel for field operations.
